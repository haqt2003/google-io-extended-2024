<template>
  <div
    ...
    class="px-5 bg-white w-full h-[54px] border-b-[1px] flex justify-between items-center fixed top-0 left-0 z-[99] xl:h-[64px] xl:px-10"
  >
    ...
    <div class="flex">
      <button
        @click="goMeeting()"
        class="hidden mr-4 xl:block w-[250px] h-[48px] gradient text-white font-semibold rounded-[30px] transition duration-300 ease-in-out"
      >
...
export default {
  ...
  setup() {
    ...
    function goAbout() {
      ...
      const title_aboutRect = title_about.getBoundingClientRect();
      const title_aboutTop = window.pageYOffset + title_aboutRect.top - 160;
      window.scrollTo({
        top: title_aboutTop,
        behavior: "smooth",
      });
    ...
    function goAgenda() {
      const title_agenda = document.querySelector(".title-agenda");
      const title_agendaRect = title_agenda.getBoundingClientRect();
      const title_agendaTop = window.pageYOffset + title_agendaRect.top - 160;
      window.scrollTo({
        top: title_agendaTop,
        behavior: "smooth",
      });
